...
void __iomem *mxs_bch_base_virt = NULL;
void __iomem *mxs_gpmi_base_virt = NULL;
void __iomem *ccm_base_virt = NULL;
void __iomem *mxs_apbh_base_virt = NULL; 
...
extern struct zb_priv_nand *zb_nand;


static void setup_gpmi_nand(void)
{
	uint32_t tmp;
	struct mxc_ccm_reg *mxc_ccm = (struct mxc_ccm_reg *)ccm_base_virt;
...
static struct mxs_dma_desc *mxs_nand_get_dma_desc(struct mxs_nand_info *info)
{
	...
	if (info->desc_index >= MXS_NAND_DMA_DESCRIPTOR_COUNT) {
		printk("MXS NAND: Too many DMA descriptors requested\n");
		return NULL;
	}
...
{
	struct mxs_bch_regs *bch_regs = (struct mxs_bch_regs *)mxs_bch_base_virt;
...
static void mxs_nand_cmd_ctrl(struct mtd_info *mtd, int data, unsigned int ctrl)
{
	...
	if (nand_info->cmd_queue_len == MXS_NAND_COMMAND_BUFFER_SIZE) {
		printk("MXS NAND: Command queue too long\n");
		return;
	}
	...
	if (ret)
		printk("MXS NAND: Error sending command\n");
...
static int mxs_nand_device_ready(struct mtd_info *mtd)
{
	...
	struct mxs_gpmi_regs *gpmi_regs =
		(struct mxs_gpmi_regs *)mxs_gpmi_base_virt;
	uint32_t tmp;
...
static void mxs_nand_read_buf(struct mtd_info *mtd, uint8_t *buf, int length)
{
	...
	if (length > NAND_MAX_PAGESIZE) {
		printk("MXS NAND: DMA buffer too big\n");
		return;
	}

	if (!buf) {
		printk("MXS NAND: DMA buffer is NULL\n");
		return;
	}
	...
	ret = mxs_dma_go(channel);
	if (ret) {
		printk("MXS NAND: DMA read error\n");
		goto rtn;
	}
				...
				int length)
{
	...
	d->cmd.address = (dma_addr_t)nand_info->data_buf_phys;//JHYOON
	...
	ret = mxs_dma_go(channel);
	if (ret)
		printk("MXS NAND: DMA write error\n");
					...
					uint8_t *buf, int oob_required, int page)
{
	...
	struct mxs_dma_desc *d;
	//uint32_t channel = MXS_DMA_CHANNEL_AHB_APBH_GPMI0 + nand_info->cur_chip;
	uint32_t channel = MXS_DMA_CHANNEL_AHB_APBH_GPMI0 + 0;  //FIXME
	...
	if (ret) {
		printk("MXS NAND: BCH read timeout\n");
		goto rtn;
	}
...
static void mxs_nand_ecc_write_page(struct mtd_info *mtd,
				struct nand_chip *nand, const uint8_t *buf, int oob_required)
{
	// FIXME- CHEKC OOB_required.. JHYOON
	struct mxs_nand_info *nand_info = nand->priv;
	struct mxs_dma_desc *d;
	uint32_t channel = MXS_DMA_CHANNEL_AHB_APBH_GPMI0 + nand_info->cur_chip;
	int ret;
	...
	ret = mxs_nand_wait_for_bch_complete();
	if (ret) {
		printk("MXS NAND: BCH write timeout\n");
		goto rtn;
	}
					...
					int page)
{
	...
	if (!nand_info->marking_block_bad) {
		printk("NXS NAND: Writing OOB isn't supported\n");
		return -EIO;
	}
...
static int mxs_nand_scan_bbt(struct mtd_info *mtd)
{
	...
	if (mtd->oobsize > MXS_NAND_CHUNK_DATA_CHUNK_SIZE) {
		printk("we do not support the NAND whose OOB size is"
	...
	//writel(BCH_CTRL_COMPLETE_IRQ_EN, &bch_regs->hw_bch_ctrl_set); //DON"T ENABLE IRQ BCH
...
int mxs_nand_alloc_buffers(struct mxs_nand_info *nand_info)
{
	...
	const int size = NAND_MAX_PAGESIZE;

  nand_info->data_buf_size = roundup(size, MXS_DMA_ALIGNMENT);
  
	nand_info->data_buf = dma_alloc_coherent(NULL,nand_info->data_buf_size,&phy,GFP_DMA);
	
printk("======================================================\n");
printk("%s dma_alloc_size 0x%08X\n", __func__, nand_info->data_buf_size);
printk("%s dma_alloc_size 0x%p\n", __func__, nand_info->data_buf);
printk("======================================================\n");
	
	if (!nand_info->data_buf) {
		printk("MXS NAND: Error allocating DMA buffers\n");
		return -ENOMEM;
	}

	memset(nand_info->data_buf, 0, nand_info->data_buf_size);

	nand_info->oob_buf = nand_info->data_buf + NAND_PAGE_SIZE;
	
	nand_info->data_buf_phys = (uint8_t *)phy;
	nand_info->oob_buf_phys = (uint8_t *)(phy + NAND_PAGE_SIZE);

	zb_nand->data_buf_addr = (unsigned int)nand_info->data_buf;
	zb_nand->data_buf_addr_phys = (unsigned int)phy;
	
	
	nand_info->cmd_buf = dma_alloc_coherent(NULL,PAGE_SIZE,&phy2,GFP_DMA);
	
	if (!nand_info->cmd_buf) {
		printk("MXS NAND: Error allocating command buffers\n");
		return -ENOMEM;
	}
	nand_info->cmd_buf_phys = (uint8_t *)phy2;

	zb_nand->cmd_buf_addr = (unsigned int)nand_info->cmd_buf;
	zb_nand->cmd_buf_addr_phys = (unsigned int)phy2;
...
int mxs_nand_init(struct mxs_nand_info *info)
{
	...
	struct mxs_bch_regs *bch_regs =
		(struct mxs_bch_regs *)mxs_bch_base_virt;
	int i = 0, j;
	
	mxs_dma_init();
	
	info->desc = kmalloc(sizeof(struct mxs_dma_desc *) *
				MXS_NAND_DMA_DESCRIPTOR_COUNT, GFP_KERNEL);
	...
	for (i = 0; i < MXS_NAND_DMA_DESCRIPTOR_COUNT; i++) {
		...
		if (!info->desc[i])
			goto err2;
		zb_nand->dma_desc[i] = (unsigned int)info->desc[i];
		zb_nand->dma_desc_phys[i] = (unsigned int)info->desc[i]->address;
	}
...
err1:
	...
	printk("MXS NAND: Unable to allocate DMA descriptors\n");
	return -ENOMEM;
}
...
// GET Base reg from Kernel Symbol For Debug	
/*
		mxs_bch_base_virt = (void __iomem *)bch_regs;
		mxs_gpmi_base_virt = (void __iomem *)gpmi_regs;*/

	if ( mxs_bch_base_virt == NULL ) mxs_bch_base_virt = ioremap_nocache(MXS_BCH_BASE,MXS_REG_MEM_SIZE);
	if ( mxs_gpmi_base_virt == NULL ) mxs_gpmi_base_virt = ioremap_nocache(MXS_GPMI_BASE,MXS_REG_MEM_SIZE);
	if ( ccm_base_virt == NULL ) ccm_base_virt = ioremap_nocache(CCM_BASE_ADDR,0x8000);
	if ( mxs_apbh_base_virt==NULL) mxs_apbh_base_virt = ioremap_nocache(MXS_APBH_BASE,MXS_REG_MEM_SIZE);

	zb_nand->mxs_bch_base_virt = (unsigned int)mxs_bch_base_virt;
	zb_nand->mxs_gpmi_base_virt = (unsigned int)mxs_gpmi_base_virt;
	zb_nand->mxs_ccm_base_virt = (unsigned int)ccm_base_virt;
	zb_nand->mxs_apbh_base_virt = (unsigned int)mxs_apbh_base_virt;

	setup_gpmi_nand();
	nand_info = kmalloc( sizeof(struct mxs_nand_info),GFP_KERNEL); 
	
	if (!nand_info) {
		printk("Func: %s, Line: %d\n", __func__, __LINE__ );
		printk("MXS NAND: Failed to allocate private data\n");
		return -ENOMEM;
	}
...
err2:
	kfree(nand_info->data_buf);
	kfree(nand_info->cmd_buf);
err1:
	kfree(nand_info);
	return err;
}
