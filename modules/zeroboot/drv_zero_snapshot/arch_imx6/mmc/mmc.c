...
static u32
mmc_bread(int dev_num, u32 start, lbaint_t blkcnt, void *dst)
{
...
static u32
mmc_bwrite(int dev_num, u32 start, lbaint_t blkcnt, const void*src)
{
	struct mmc *host = find_mmc_device(dev_num);
	struct mmc_cmd cmd;
	struct mmc_data data;
...
u32 movi_write(u32 start, lbaint_t blkcnt, void *src)
{
	return mmc_bwrite(0, start, blkcnt, src);
}

u32 movi_read(u32 start, lbaint_t blkcnt, void *dst)
{
...
//printf("--%s(%d)  [Test for SD version 2]\n",__func__,__LINE__);
	/* Test for SD version 2 */
	err = mmc_send_if_cond(host);

//printf("--%s(%d)  [Now try to get the SD card's operating condition]\n",__func__,__LINE__);
	/* Now try to get the SD card's operating condition */
	err = mmc_send_app_op_cond(host);

//printf("--%s(%d)  [If the command timed out, we check for an MMC card]\n",__func__,__LINE__);
